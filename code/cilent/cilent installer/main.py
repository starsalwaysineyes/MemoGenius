...
def main():
    ...
    def load():
        def loadthread():
            ...
            #设置标题
            result_window.title("内容纪要")
            
            #创建文本框
            text_box=tk.Text(result_window)
            text_box.pack()
            # 获取文本内容
            text_content = lst[1]  # lst[1] 存储了文本内容
            
            # 在文本框中插入文本内容
            text_box.insert(tk.END, text_content)
        threadLoad=threading.Thread(target=loadthread)
        threadLoad.start()
    ...
    """button4=tk.Button(window,text="测试获取", font=("Helvetica", 18),fg="white",bg="lightblue",command=load)
    button4.place(relx=0.52, rely=0.3, anchor="center")"""
